...
(function($){
    $.fn.smartWizard = function(action) {
        ...
        return this.each(function(){
                ...
                var contentWidth = 0;
                var loader,msgBox,elmActionBar,elmStepContainer,btNext,btPrevious;
                ...
                function init(){
                  ...
                  $('.button-class').click(function(){
                  	var whatToShow = $(this).parent().attr('id');
                  	doForwardProgress();
                  	
                   	if(whatToShow == 'standard'){
                  		$('#standard-filter').show('slide');
                  		$('#mCuststand').show('slide');
                  			$('#cCustcarb').hide();                  		
                  			$('#carbon-filter').hide();
                  	}else{
                    ...
                    if(options.transitionEffect == 'slide'){
                      ...
                      $($(curStep, obj).attr("href"), obj).fadeOut("fast",function(e){
                        ...
                        if(stepIdx == curStepIdx){
                            ...
                            $($(curStep, obj).attr("href"), obj).animate({left:curElementLeft},"slow",function(e){
                        ...
                        $($(selStep, obj).attr("href"), obj).animate({left:nextElmLeft2},"slow",function(e){
                          curStepIdx =  stepIdx;                        
                          SetupStep(curStep,selStep);                      
                        });
                    } else{
                ...
                function adjustButton(){
                  if(!options.cycleSteps){                
                    if(0 >= curStepIdx){
                      $(btPrevious).hide().addClass("buttonDisabled");
                    }else{
                      $(btPrevious).show().removeClass("buttonDisabled");
                    }
                    if((steps.length-1) <= curStepIdx){
                      $(btNext).hide().addClass("buttonDisabled");
                    }else{
                      $(btNext).show().removeClass("buttonDisabled");
                    }
                  }                
                }
